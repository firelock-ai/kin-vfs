...
| Facet | Value |
|---|---|
| **Version source** | `[workspace.package] version` in the root `Cargo.toml` (currently `0.2.2`). Every workspace member inherits it via `version.workspace = true`; the excluded `kin-vfs-fuse` crate remains separately versioned. |
