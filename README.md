...
| FUSE and NFS mounts | Optional source-build features, not enabled in the prebuilt `kin-vfs` binary shipped with Kin today. The NFS mount is writable: a write through it becomes a Kin change. FUSE is read-only. |
...
FUSE is read-only. On macOS it requires FUSE-T or macFUSE; on Linux it requires libfuse.
