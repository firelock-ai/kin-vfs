...
| Native Windows | **Not shipped for VFS projection, though the ProjFS provider is now proven against a live filesystem.** The Kin archive still carries no Windows projection files, and no shipped binary starts the provider, so a native Windows install has no projection today. What changed is the evidence behind it: the `ProjFS live proof (windows-latest)` CI job virtualizes a real directory over a real daemon on every run, then reads it, lists it, and writes to it from a separate PowerShell process. Use WSL2 with a Linux distribution that provides glibc 2.39 or newer for the supported Windows-hosted path. |
...
- **`crates/kin-vfs-shim`:** The injected `cdylib` interception layer for Linux and macOS, plus the Windows ProjFS provider. The provider is exercised live in CI but is not started by any shipped binary, so it is not yet a Windows projection path a user can run.
